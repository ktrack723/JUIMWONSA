...
export const TUNING = {
  ...
  // 전우애 — **사건을 줄이지 않는다. 보고를 줄인다.**
  //
  // 오래도록 이 축은 「갈등을 흡수하는 완충재」였다: 전우애가 높으면 잔사건이 덜 나고, 큰 사고의
  // 문턱이 뒤로 밀리고, 넘어서도 덜 번졌다. 그러면 끈끈한 부대는 **실제로 안전한 부대**가 된다 —
  // 실측이 그걸 그대로 찍었다(해병 40일 방치, 사고 0건, 완봉).
  //
  // 그건 사실이 아니다. 사내다움이 두꺼운 부대에서 일어나는 일은 「사고가 안 나는 것」이 아니라
  // **사고가 안 올라오는 것**이다. 고자질이 제일 큰 죄인 자리에서 병사는 의무대에 안 가고,
  // 분대장은 위에 안 올리고, 주임원사의 장부는 조용하다. 절대 건수는 오히려 **많다** —
  // 마초가 사건을 만드니까(roll.machoPer). 조용한 것은 부대가 아니라 장부다.
  // 공군은 반대다: 손가락에 밴드 하나 붙여도 그날 서면으로 올라간다. 시끄러운 장부는
  // 그 부대가 더 위험하다는 뜻이 아니라 **숨은 것이 없다**는 뜻이다.
  //
  // 그래서 이 축은 사고 롤에서 통째로 빠지고 두 자리로 옮겨 갔다:
  //   ① report — 사건 하나가 주임원사에게 올라올 확률. 안 올라온 것은 화면에 없다(콜도 없다)
  //   ② buried — 안 올라간 것이 쌓이는 더미. **위험을 미는 것은 이제 이 더미다**
  // 「겉으론 잔잔하다가 한 번에 터진다」가 이 둘의 곱이다. 중립은 5.
  comrade: {
    neutral: 5,
    // 보고 — 침묵의 압력이 셀수록 낮다. 마초도 같이 본다(고자질을 죄로 만드는 것은 마초다).
    // 중립 **아래로는 안 민다**: 서로 남인 부대라고 보고가 100%를 넘을 수는 없고, 안 올리는
    // 이유가 없으면 그냥 올라온다. 이 축은 침묵의 압력이지 투명도의 눈금이 아니다.
    report: {
      base: 0.95,        // 아무 압력이 없으면 거의 다 올라온다 — 공군 자리
      perComrade: 0.09,  // 중립 초과 전우애 1당
      perMacho: 0.05,    // 중립 초과 마초 1당
      majorMul: 0.5,     // 큰 사건은 절반만큼만 숨는다 — 사람이 실려 가면 못 덮는다
      min: 0.1,          // 아무리 눌러도 열에 하나는 새어 나온다
    },
    // 묻힌 더미 — 이 게임에서 유일하게 **개수조차 안 보이는** 상태다(가라는 개수는 보인다).
    // 쌓일수록 큰 사고의 문턱이 앞당겨지고, 넘은 뒤의 번짐 폭이 커지고, 같은 자리에서 잔사건이
    // 또 난다(아무도 안 끊었으니까). 터지면 **비워진다** — 그날 올라오는 것은 오늘 것 하나가
    // 아니라 그동안의 전부다. 그게 「그러다 크게 터진다」의 기계판이다.
    buried: {
      cap: 8,                // 위험 계산이 세는 최대치. 상한이 없으면 100일이 확정 파멸이다
      // **묻힌 것도 삭는다.** 상한만으로는 부족하다 — 상한은 위험을 자를 뿐이고 더미 자체는
      // 계속 자라서, 침묵이 두꺼운 부대는 부임 3주면 상한에 붙어 100일 내내 거기 눌러앉는다
      // (실측: 더미 평균 19.6 · 완주율 4%). 그러면 「쌓이다 터진다」가 아니라 「처음부터 열려
      // 있다」가 되고, 이 축이 다시 저주가 된다.
      // 실제로도 안 올라간 일의 대부분은 아무 일도 안 일어난 채 지나간다 — 그때 그 자리에
      // 있던 사람들이 전역하고, 아무도 그 얘기를 안 하고, 그것으로 끝이다.
      // 그래서 이 축은 **평형**이지 시계가 아니다: 묻는 속도가 삭는 속도를 넘는 구간에서만
      // 더미가 자라고, 나쁜 구간이 길어질수록 문턱이 앞으로 걸어온다.
      fadeDays: 6,           // 묻힌 지 이만큼 지난 것은 조용히 지나간 일이 된다
      // **그리고 더미는 갈등을 민다.** 이게 없으면 이 축이 통째로 죽는다 — 실측으로
      // 알았다: 더미가 문턱을 앞으로 당겨 놔도 100일 중 **문이 열린 날이 0.1일**이었다.
      // 이유가 분명했다. 갈등을 미는 것은 확전 판정인데, 판정은 올라온 사건에만 붙는다.
      // 그러니 침묵이 두꺼운 부대는 사건이 세 배로 나면서 **갈등 바늘까지 더 조용했다.**
      // 안 올린 일이 부대를 편하게 만들어 주는 셈이라, 고치려던 것이 그대로 되돌아왔다.
      // 실제로 그 자리에 남는 것이 이것이다: 아무도 말 안 한 일이 몇 건씩 깔려 있는
      // 생활관은 말이 줄고 눈치가 늘고 사소한 것에 날이 선다. 그게 갈등이다.
      // 미는 방식이 중요하다. 「쌓였으면 매일 +1」로 해 봤더니 갈등이 천장까지 행진해서
      // 눌러앉았다(실측: 문 열린 날 37/100 · 사고 8.95건 · 완주 0%). 회복이 이틀에 한 칸인데
      // 미는 힘이 매일 한 칸이면 그건 압력이 아니라 래칫이다.
      // 그래서 **더미는 갈등의 바닥을 만든다** — 자기 크기까지만, 하루 한 칸씩.
      // 여섯 건이 깔린 부대의 갈등은 6 아래로 안 내려가고, 6에서 멈춘다. 더미가 삭거나
      // 터져서 비면 바닥이 같이 내려가고, 그때부터 갈등도 제자리로 걸어 돌아간다.
      // 이 규칙 하나가 「쌓이다 열리고, 터지고, 닫힌다」의 순환을 만든다: 문턱은 더미 1건당
      // 0.4씩 앞으로 오고 바닥은 1씩 올라오므로, 둘이 만나는 자리가 대략 여섯 건이다.
      // 그 아래로는 아무 일도 안 일어나고 — 계기판만 조용히 오른다 — 그 위에서 문이 열린다.
      //
      // **계기판이 오르는데 왜 오르는지 장부 어디에도 안 적혀 있는 것**이 이 게임에서 침묵의
      // 값이고, 주임원사에게 남은 유일한 단서이기도 하다. 개입도 판정도 없었는데 갈등이
      // 안 내려가면, 그건 그가 못 들은 일이 부대에 깔려 있다는 뜻이다.
      openPer: 0.4,          // 묻힌 것 1건당 큰사고 문턱(갈등)이 이만큼 **앞당겨진다**
      bigPer: 0.08,          // 1건당 문턱 초과분 가중
      smallPer: 0.005,       // 1건당 잔사건 위험 — 안 끊긴 것은 같은 자리에서 또 난다
      surfacePerInspect: 1,  // 들이닥치면 그 자리에서 몇 건까지 드러나는가
      keep: 40,              // 저장이 들고 있는 최대 건수
    },
    // 전우애는 **부대 분위기로부터 개인을 지키는 방패**이기도 하다. 문턱을 이만큼 민다:
    // 끈끈한 부대는 분위기가 어지간히 나빠도 사람이 안 무너지고, 조금만 좋아져도 회복한다.
    // 서로 남인 부대는 반대다. 다만 이 방패는 **분위기에만** 붙는다 — 묻힌 사건이 연루자를
    // 깎는 것은 그대로다. 덮는 것은 그 사람이 안 다쳤다는 뜻이 아니다.
    mentalPer: 0.4,
...
/**
 * 사건 하나가 **위로 올라갈** 확률. 이 게임에서 전우애가 하는 일의 절반이 여기다.
 *
 * 압력은 전우애와 마초가 같이 만든다 — 끈끈해서 안 올리고, 사내다워서 안 올린다.
 * 중립 아래로는 안 민다: 안 올릴 이유가 없는 부대에서는 그냥 올라온다.
 * 큰 사건은 절반만큼만 숨는다(majorMul). 사람이 실려 나가면 덮을 수가 없다 —
 * 다만 실려 나가기 **전까지**는 덮인다. 그 차이가 이 게임의 실패 모양이다.
 *
 * 성향을 안 주면 전부 중립이라 base가 그대로 나온다(테스트·옛 부대 데이터).
 */
export function reportChance(tier, { comrade, macho } = {}) {
  const C = TUNING.comrade, R = C.report;
  const silence = Math.max(0,
    ((comrade ?? C.neutral) - C.neutral) * R.perComrade
    + ((macho ?? C.neutral) - C.neutral) * R.perMacho);
  const p = R.base - silence;
  return Math.max(R.min, Math.min(1, tier === 'major' ? 1 - (1 - p) * R.majorMul : p));
}

/**
 * 묻힌 더미가 미는 위험. **부대 static이 아니라 캠페인 상태다** — 여기가 예전과 제일
 * 다른 자리다. 전우애는 이제 이 더미가 얼마나 빨리 쌓이는지만 정하고, 위험을 실제로
 * 미는 것은 쌓인 개수다. 그래서 같은 부대라도 100일 중 언제냐에 따라 위험이 다르다.
 *   open  — 큰 사고가 열리는 갈등 문턱. 묻힌 것이 많을수록 **앞당겨진다**
 *   scale — 문턱 초과분의 가중 배수. 묻힌 것이 많을수록 크게 번진다
 *   small — 작은 사건 위험 가산. 안 끊긴 것은 같은 자리에서 또 난다
 * 더미가 비면(사고로 한 번 터졌거나, 애초에 다 올라오는 부대거나) 기준선 그대로다.
 */
export function buriedEffect(buried = 0) {
  const B = TUNING.comrade.buried, R = TUNING.roll;
  const n = Math.max(0, Math.min(B.cap, buried));
  return {
    open: R.big.open - n * B.openPer,
    scale: 1 + n * B.bigPer,
    small: n * B.smallPer,
  };
}
 ...
 */
export function incidentRisk({ gara, conflict, minMental = 10, overreach = 0, heat = 0, crimes = 0, buried = 0 }, { macho, difficulty }) {
  const R = TUNING.roll, M = TUNING.mental, G = TUNING.gara;
  const C = buriedEffect(buried);
  ...
  const small = Math.max(0,
    ...
    + C.small                                       // 묻어 둔 것이 같은 자리에서 또 난다
    - (conflict >= R.suppressAt ? R.suppress : 0));
  // 묻힌 더미가 문턱을 앞당기고, 넘어선 뒤의 번짐 폭도 정한다 —
  // 같은 갈등 8이라도 아무것도 안 묻은 부대에서는 아직 안 열리고, 여섯 건을 묻어 둔
  // 부대에서는 이미 한참 전에 열려 있었다. 계기판은 그 자리가 어디로 갔는지 말해 주지 않는다.
  ...
  // 맞고 있는 사람이 있으면 그 사람의 사고는 이미 열려 있다. 아무것도 안 묻어 둔 부대에서
  // 큰 사고가 나는 거의 유일한 길이기도 하다: 갈등 문턱은 기준선(8)에 그대로 있으니까.
